...
| -------- | ------------------------------------------------------------ |
| 物理内存 | 硬件内存                                                     |
| 虚拟内存 | 操作系统为每个进程提供的一个独立虚拟地址空间, 可映射为物理内存 |
|          | 虚拟的, 连续的, 进程间独立, 分页/分段管理.                   |
|          |                                                              |
|          | **虚拟内存映射到物理内存的过程**                             |
| 1        | **页表(Page Table)**: 每个进程都有自己的页表，页表包含虚拟地址到物理地址的映射关系 |
| 2        | **页目录(Page Directory)**: 在32位系统中，页表分为多个页目录来管理 |
| 3        | **页帧(Page Frame)**: 物理内存被划分成固定大小的页帧(通常是4KB) |
| 4        | **页表项(Page Table Entry, PTE)**: 页表中的每个条目指向一个页帧 |
| 5        | **地址转换**: 当CPU访问虚拟地址时，通过页表查找到对应的物理地址 |

### 虚拟内存模型

C++ Linux 中ELF文件常见段有哪些?

| 段       | 名称       | 读写 | 存储内容                      |
| -------- | ---------- | ---- | ----------------------------- |
| **.text** | 代码段     | **只读+可执行** | 可执行代码, 包括函数体和指令. |
| **.data** | 数据段     | **读写** | 已初始化的全局变量/静态变量   |
| **.bss** | BSS段      | **读写** | 未初始化的全局变量/静态变量, **不占空间** |
| **.rodata** | 只读数据段 | 只读 | 常量/字符串字面值 |
| **.comment**          | 注释段 | 只读 | 注释信息, 对程序执行无影响 |
| .symtab | 符号表段 | 只读 | 符号表, 变量名\|函数等 |
| .strtab | 字符串表段 | 只读 | 符号表和调试信息中的字符串数据 |
| .rel.text\|.rela.text | 重定位段 | 只读 | 重定位信息, 链接\|加载到内存中修改地址 |
| **.dynamic** | 动态链接信息段 | **读写** | 动态链接器所需的信息 |
| .init\|.fini | 初始化\|终结代码段 | 只读 | 程序初始化和终结时需要执行的代码 |
...
| **用法**         | 代码示例                                             |
| ---------------- | ---------------------------------------------------- |
| `new/delete`     | `T* obj = new T(); delete obj;`                      |
| `new[]/delete[]` | `T* arr = new T[size]; delete[] arr;`                |
| `malloc/free`    | `void* ptr = malloc(size); free(ptr);`               |
| `calloc/free`    | `void* ptr = calloc(num, size); free(ptr);`          |
| `realloc/free`   | `void* ptr = realloc(old_ptr, new_size); free(ptr);` |
|                  |                                                      |
| 相同点           | 说明                                                 |
| 目的             | 在堆上, 动态内存分配与释放                           |
| 需要手动释放     | 需手动释放, 否则导致内存泄漏; 即分配和释放配对使用.  |
| 失败处理         | 内存分配失败, 返回空指针/或抛出异常                  |

**差异对比**

| 特性          | `new/delete`              | `new[]/delete[]`          | `malloc/free`      | `calloc/free`  | `realloc/free`            |
| ------------- | ------------------------- | ------------------------- | ------------------ | -------------- | ------------------------- |
| **用于**      | 对象                      | 数组                      | 指定字节数         | 分配并初始化   | 调整已分配内存大小        |
| **初始化**    | 构造                      | 构造                      | 不初始化           | 初始化为零     | 不初始化新分配内存        |
| **返回值**    | 特定类型指针              | 特定类型指针              | void*              | void*          | void*                     |
| **类型安全**  | 是                        | 是                        | 否(需手动计算大小) | 否             | 否                        |
| **构造/析构** | 是                        | 是                        | 否                 | 否             | 否                        |
| **错误处理**  | 抛出异常 `std::bad_alloc` | 抛出异常 `std::bad_alloc` | 返回 `nullptr`     | 返回 `nullptr` | 返回 `nullptr` / `原指针` |
| **性能**      | 较慢, 带构造/析构开销     | 较慢, 带构造/析构开销     | 一般较快           | 一般较快       | 一般较快                  |
...
|                | 常见内存问题说明                                             |
| -------------- | ------------------------------------------------------------ |
| 未初始化内存   | Use of Uninitialized Memory, 使用未初始化内存/变量           |
|                | 未初始化内存可能包含任何值                                   |
| 空指针         | Null Pointer Dereference, 访问空指针/解引用空指针            |
| 野指针         | Dangling Pointer, 指向已释放或无效内存的指针.                |
|                | 悬空指针从合法状态 变为非法状态; 野指针一直处于非法状态.     |
| 错误释放       | wild free,  **释放(未分配/已释放/静态/栈)内存**;             |
| 多次释放       | double free, 同一块内存释放两次/多次                         |
| **内存泄漏**   | **memory leak, 申请未释放, 申请与释放不配对**                |
| **内存碎片化** | **Memory fragmentation, 频繁的分配释放导致**                 |
| 内存越界       | memory overrun, 访问超出分配内存范围区域, (数组/指针/读/写)越界 + 缓冲器溢出 |
| 缓冲区溢出     | Buffer Overflow, 写入超出缓冲区内存范围区域, 栈/堆/全局/字符串 缓冲区溢出. |
| 读取非法内存   | read invalid memory, 本质上属于内存越界                      |
| 栈溢出         | stack overflow, 递归太深/死循环/(局部变量过多/过大)          |
|                |                                                              |
|                | **可能导致的后果及内存问题**                                 |
| **程序崩溃**   | 严重内存错误: 访问空指针/访问野指针/错误释放/多次释放/...等  |
| **性能下降**   | 内存泄漏/内存碎片, 可导致内存耗尽->性能下降->系统变慢->系统崩溃, |
|                | 垃圾回收/碎片整理有额外开销.                                 |
| **数据损坏**   | 内存越界/缓冲区溢出/错误释放/多次释放, 可能导致**逻辑错误**  |
| **安全漏洞**   | 缓冲区溢出/内存越界, 攻击者可利用它覆盖程序内存并控制程序执行流程 |
| **未定义行为** | 不稳定/不可预测, 未初始化内存/野指针//错误释放/读物非法内存/内存越界 |
| **段错误**     | 访问非法内存时, 操作系统引发的错误                           |
|                |                                                              |
|                | **段错误说明**                                               |
| 段错误         | 访问非法(受保护/无效的)内存地址而引发的错误.                 |
|                | 操作系统会阻止非法访问, 并产生一个段错误型号(SIGSEGV).       |
|                | **可能导致段错误的原因**                                     |
| 指针           | 空指针解引用; 野指针; 未初始化指针; 非法指针运算(使指针指向无效区域) |
| 越界           | 数组越界/缓冲器溢出/栈溢出/                                  |
| 非法内存       | 访问未映射内存/未分配内存                                    |
...
| -------- | -------------------------------------- |
| 数组     | 索引超出实际大小                       |
| 字符串   | strcpy, strcat 等 未检查边界就进行操作 |
| 动态内存 | 请求内存少于实际所需                   |
...
int main() {
    char buffer[10];
    strcpy(buffer, "This string is too long for the buffer"); // 缓冲区溢出
    return 0;
}
```
...
| 初始化   | 变量/内存/指针 使用前初始化                                  |
| 指针     | 指针使用前判空, 使用后置空                                   |
| 泄漏     | 动态内存分配要配对; new/delete, new[]/delete[], malloc/free  |
| 智能指针 | 避免手动管理内存, 避免使用裸指针                             |
| 结构体   | 头文件中结构体大小和顺序尽量不调整, 如需调整要做到全量更新   |
| RAII     | 资源获取即初始化, 避免资源泄漏(文件句柄/锁/动态内存等)       |
| 数组     | 安全访问, 防止越界.                                          |
| 线程安全 | 共享资源访问, 注意锁保护/同步机制防止竞争/内存破坏           |
| 内存对齐 | 某些平台有严格对齐要求, 不合适的对齐可能导致性能下降或硬件错误. |
| 内存碎片 | 避免频繁的申请与释放内存                                     |
|          |                                                              |
|          | **如何避免栈溢出**                                           |
| 栈       | 用于存储函数调用、局部变量和临时数据的数据结构               |
| 1        | 避免无限递归/深度递归, 确保有明确退出条件.                   |
| 2        | 小心处理局部大变量和大数组, 可使用stl来进行处理.             |
| 3        | 了解平台栈大小限制, 并适当调整.                              |
| 4        | 使用编译器栈保护选型, 如栈溢出保护(Stack Protection)或栈canary |
...
| 工具      | 说明                                                         |
| --------- | ------------------------------------------------------------ |
| Valgrind  | 强大的内存调试, 内存泄漏检测以及性能分析工具.                |
|           | 常用的`Memcheck`检测内存泄漏和内存错误                       |
| ASan      | Google开发的快速内存错误检测器, 可以集成在Clang/GCC/编译器中. |
|           | 用于检测堆栈缓冲区溢出、使用后释放(dangling pointers)、未初始化内存读取等问题. |
|           | 编译时添加`-fsanitize=address`选项即可启用AddressSanitizer   |
| gdb       | 调试分析                                                     |
| perf      | 强大的性能分析工具, 分析程序CPU使用情况,缓存命中/未命中,分支预测等 |
|           | 也可以通过分析内存访问模式来辅助内存问题的诊断               |
| pmap      | 命令行工具, 显示进程的内存映射;  /proc/[pid]/smaps 进程内存使用详细信息. |
| cppcheck  | 静态分析工具, 用于检测C++代码问题: 包括内存泄漏/缓冲区溢出/未初始化的变量等 |
| LeakTrace | 轻量级的C++内存泄漏检测库, 它可以集成到你的项目中, 用于在运行时检测内存泄漏. |
